...
3、该类对应的java.lang.Class对象没有任何地方被引用，无法在任何地方通过反射访问该类的方法。<br />
如果以上三个条件全部满足，jvm就会在方法区垃圾回收的时候对类进行卸载，类的卸载过程其实就是在方法区中清空类信息，java类的整个生命周期就结束了。用户自定义的类加载器才能被回收，jdk自带
...
### 打破双亲委派模型
#### SPI
双亲委派模型是解决了类统一加载的问题，保证了类不会被重复加载，也考虑到了一些安全因素，java核心api中的类不会被随意篡改。然后为什么要打破这个模型呢，因为正常情况下都是用户代码去调用jdk的基础类，但存在一些特殊情况是基础类去调用用户代码的场景，比如打破双亲委派的其中一种常见情况spi就是jdk去调用用户代码去加载类，我们常见的java.sql.driver就是spi的一种，因为jdk不知道你使用的是什么具体数据库，所以不能在jdk基础类里面硬编码加载你的数据库驱动，所以提供了驱动接口由各厂商去实现，并且提供上下文类加载器的设置可以拿到当前实现类加载器（可以设置，默认是应用程序类加载器），jdk中的基础类drivermanager会去扫描包中的驱动，然后用获得当前类的加载器去加载驱动（正常情况是应用程序类加载器），因为drivermanager是bootstrap加载器加载，按照双亲委派原则bootstrap类加载器是拿不到应用程序加载器的，当目前这种情况来看就是打破了这个原则，出现了启动类加载器使用应用程序加载器去加载的情况
#### OSGI
osgi是模块热部署，每个模块都有他自己的类加载器，更换模块的时候连同他的类加载器一起更换，此时的类加载器就不是双亲委派模型的树状结构了，而是一种网状结构，会按照一定的网状查找规则使用对应的类加载器进行加载
#### JDK9模块化
双亲委派模型会打包加载一些你没使用到的类，jdk9模块化之后对rt.jar进行拆分了很多模块，每个模块都有自己的类加载器，每个类加载器各司其职加载自己所负责的类，根据应用程序使用到哪些类从而使用对应的类加载器进行加载，所以也违反了双亲委派模型
